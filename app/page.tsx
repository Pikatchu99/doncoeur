...
export default function Page() {
  ...
  function nextQuestion() {
    if (!questions[question].value) return
    if (question < questions.length - 1) setQuestion(question + 1)
    else {
      const ok = Number(age) >= 18 && Number(age) <= 65 && Number(weight) >= 50
      setEligibility(
        ok
          ? 'Merci ' +
              firstName +
              ', vous pouvez faire le premier pas. Rendez-vous dans un centre proche de vous.'
          : 'Merci ' +
              firstName +
              ', le centre vous confirmera votre aptitude avec vous, sur place.',
      )
    }
  }
    ...
    <main className="bg-background text-foreground min-h-screen overflow-hidden">
          ...
          <div className="bg-card text-card-foreground rounded-[2rem] p-6 shadow-2xl sm:p-8">
            ...
            {eligibility ? (
              <div className="result-pop py-12">
                ...
                <h3 className="mt-6 font-serif text-3xl font-bold">{eligibility}</h3>
                <a
                  href="#centres"
      ...
      <footer className="border-border bg-card border-t">
        ...
        <div className="border-border text-muted-foreground flex flex-col gap-2 border-t px-5 py-5 text-xs sm:flex-row sm:items-center sm:justify-between sm:px-8">
          <p>© 2026 DonCœur</p>
          <p>Conçu par <a href="https://github.com/Pikatchu99">Yémalin</a></p>
        </div>
      </footer>
    </main>
  )
